...
--컬럼 지정 - AUTH_ID는 양쪽에 다 있기 떄문에, 출력할때, 테이블명.컬럼명 으로 지정해야한다.
...
--RIGHT OUTER JOIN - 오른쪽 테이블이 기준이 되고, 오른족 테이블은 다나온다.
--붙을 수 없는 데이터는 NULL로 처리됨
SELECT *
FROM INFO I
...
SELECT  EMPLOYEE_ID 사원번호, FIRST_NAME 이름, SALARY 급여, DEPARTMENT_NAME 부서명, STREET_ADDRESS 근무지
FROM EMPLOYEES E
JOIN DEPARTMENTS D
ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
JOIN LOCATIONS L
...
JOIN JOBS J
ON E.JOB_ID = J.JOB_ID
WHERE JOB_TITLE = 'Stock Manger' or JOB_TITLE = 'Stock Clerk';
