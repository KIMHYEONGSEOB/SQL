...
--LIKE 연산자
SELECT FIRST_NAME, HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE LIKE '03%'; --03으로 시작하는
SELECT FIRST_NAME, HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE LIKE '%01'; --01으로 끝나는
SELECT FIRST_NAME, HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE LIKE '%05%'; --05가 포함되는
SELECT FIRST_NAME, HIRE_DATE FROM EMPLOYEES WHERE FIRST_NAME LIKE '_ar%'; --는 자리수
SELECT FIRST_NAME, HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE LIKE '___05%';

--IS NULL, IS NOT NULL
SELECT * FROM EMPLOYEES WHERE COMMISSION_PCT = NULL; -- 불가능
...
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR' AND SALARY >= 5000; //AND가 먼저 연산
...
--ORDER BY 구문
SELECT FIRST_NAME, SALARY FROM EMPLOYEES ORDER BY SALARY ASC; -- ASC가 기본값
SELECT FIRST_NAME, SALARY FROM EMPLOYEES ORDER BY SALARY DESC; -- 내림차순
SELECT FIRST_NAME, DEPARTMENT_ID, SALARY FROM EMPLOYEES ORDER BY DEPARTMENT_ID, SALARY DESC; --먼저 부서아이디로 정렬, 동순위에 대해서 SALARY 내림차순 정렬

SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEES ORDER BY 연봉 DESC; --엘리어스를 정렬에서 사용할 수 있다.
SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEES WHERE 연봉 >= 10000; --WHERE 사용 X
