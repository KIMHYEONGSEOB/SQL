--문자열 함수
SELECT 'abcDEF' 기본값, LOWER('abcDEF') LOWER, UPPER('abcDEF') UPPER, INITCAP('abcDEF')INITCAP FROM DUAL;
...
--SUBSTR(타겟, 시작위치, 자를문자갯수(인덱스아님!)) - 문자열 자르기, CONCAT - 문자열 붙이기
SELECT FIRST_NAME, SUBSTR(FIRST_NAME, 1, 3) FROM EMPLOYEES;
SELECT CONCAT(FIRST_NAME, LAST_NAME), FIRST_NAME || LAST_NAME FROM EMPLOYEES;

--LPAD - 왼쪽공백을 특정 값으로 채움
--RPAD - 오른쪽
SELECT LPAD(FIRST_NAME, 10, '*') FROM EMPLOYEES;
SELECT RPAD(FIRST_NAME, 10, '-') FROM EMPLOYEES;

--TRIM - 양쪽 공백 제거, LTRIM - 왼쪽에서제거, RTRIM - 오른쪽에서제거
SELECT  '  HELLO WORLD  ' 기본값, TRIM('  HELLO WORLD  ') TRIM, LTRIM('  HELLO WORLD  ') LTRIM, RTRIM('  HELLO WORLD  ') RTRIM FROM DUAL;
...
SELECT CONCAT(CONCAT(FIRST_NAME, ' '),LAST_NAME) 이름, REPLACE(HIRE_DATE, '/','') 입사일 FROM EMPLOYEES ORDER BY 이름;
